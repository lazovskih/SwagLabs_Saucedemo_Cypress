...
describe("Checkout flow", () => {
  ...
  it("Completes checkout for a selected product", () => {
    ...
    const selectedProducts: Product[] = [products[1]!, products[2]!, products[3]!];

    productsPage.addProductsToCart(selectedProducts);
    productsPage.getCartCount().then((count) => {
      expect(count).to.equal(selectedProducts.length);
    });
  ...
  it("Completes checkout and verifies totals for multiple selected products", () => {
    // Add multiple products to cart
    const selectedProducts: Product[] = [products[1]!, products[2]!];
    productsPage.addProductsToCart(selectedProducts);

    productsPage.getCartCount().then((count) => {
      expect(count, "Items count on cart badge").to.equal(selectedProducts.length);
    });
    ...
    const subtotal = (products: Product[]): number => products.reduce((sum, { Price }) => sum + Price, 0);
    const expectedSubtotal = subtotal(selectedProducts).toFixed(2);
